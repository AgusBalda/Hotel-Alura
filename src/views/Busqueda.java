	...
	private void LlenarTablaReservas() {
		...
		try {
			for (Reserva reservas : reserva) {
				modelo.addRow(new Object[] { reservas.getId(), reservas.getFechaEntrada(), reservas.getFechaSalida(), reservas.getValor(), reservas.getFormaDePago() });
			}
		} catch (Exception e) {
			throw e;
		}
	...
	private void LlenarTablaHuespedes() {			       
		...
		try {
			for (Huespedes huespedes : huesped) {
				modeloHuesped.addRow(new Object[] { huespedes.getId(), huespedes.getNombre(), huespedes.getApellido(), huespedes.getFechadenacimiento(), huespedes.getNacionalidad(), huespedes.getTelefono(), huespedes.getIdreserva() });
			}
		} catch (Exception e) {
			throw e;
		}
